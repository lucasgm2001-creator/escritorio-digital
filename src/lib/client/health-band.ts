// Saúde do Cliente (CLIENT-004 · evoluída em CLIENT-005). Camada de APRESENTAÇÃO: classifica dados REAIS
// já apurados (status, semanas pendentes, tempo sem atividade, tempo sem reunião) numa faixa visual. Regra
// simples e ISOLADA aqui para evoluir depois — não recalcula negócio, só mapeia para rótulo/cor.
...
const BANDS: Record<ClientHealthKey, Omit<ClientHealthBand, 'key'>> = {
  ...
  atencao:   { label: 'Atenção',   cls: 'text-amber-300 bg-amber-900/20 border-amber-700/40',        dot: 'bg-amber-400',   hint: 'Pendência ou sem contato' },
  critica:   { label: 'Crítica',   cls: 'text-red-300 bg-red-900/20 border-red-700/40',              dot: 'bg-red-400',     hint: 'Inativo ou em atraso' },
}

export function clientHealthBand(input: {
  status: string
  semanasPendentes: number
  daysSinceActivity: number | null
  daysSinceLastMeeting?: number | null
}): ClientHealthBand {
  const noMeetingLong = input.daysSinceLastMeeting != null && input.daysSinceLastMeeting > 45
  ...
  else if (noMeetingLong) key = 'atencao'
