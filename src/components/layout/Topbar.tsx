...
function LiveClock({ timezone, label, short, primary }: { timezone: string; label: string; short: string; primary?: boolean }) {
  ...
  const labelCls = `text-[10px] uppercase tracking-wide whitespace-nowrap ${primary ? 'text-lime-fg font-semibold' : 'text-bento-muted'}`
  return (
  ...
  return (
    <header className="min-h-[56px] pt-safe border-b border-[#2d3748] bg-[#0d1117] flex items-center px-4 gap-4 shrink-0">
      ...
      <div className="flex items-center gap-2 shrink-0 lg:hidden">
        ...
        <span className="hidden sm:inline font-display font-bold text-foreground text-[15px] tracking-tight whitespace-nowrap">Escritório Digital</span>
      </div>
      ...
      <div className="ml-auto flex items-center gap-2 sm:gap-4 min-w-0">
        <div className="flex items-center gap-2 lg:gap-3.5 border-r border-[#2d3748] pr-3 sm:pr-4 shrink-0">
          ...
          <div className="hidden min-[360px]:block"><LiveClock timezone="America/New_York" label="EUA Leste" short="NY" /></div>
          <div className="hidden min-[360px]:block"><LiveClock timezone="America/Denver" label="EUA Mont." short="DEN" /></div>
