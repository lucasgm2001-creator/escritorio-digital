...
// Módulo em ROADMAP: cartões curtos (Status / Objetivo / Próxima etapa). Sem textos longos, sem back-link
// (a navegação vem do cabeçalho da Administração — rail no desktop, bottom sheet no mobile).
export function AdminSectionView({ sectionKey }: { sectionKey: AdminSectionKey }) {
  ...
  const cards: { label: string; value: string }[] = [
    ...
    { label: 'Objetivo', value: section.tagline },
    { label: 'Próxima etapa', value: section.planned[0] ?? 'Definir escopo' },
  ]
