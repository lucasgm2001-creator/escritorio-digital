...
// Error boundary da Administração (HOTFIX-REMUNERACAO-CRASH). Faltava: o grupo (dashboard) já tinha o seu
// (src/app/(dashboard)/error.tsx), mas /admin/* não — então qualquer exceção de render numa página da
// Administração (ex.: Remuneração) caía no fallback padrão do Next e mostrava o cru "Application error: a
// client-side exception has occurred", sem recuperação. Aqui a área degrada com estado honesto + "Tentar
// novamente" (reset()). Mesma linguagem/DS do boundary do dashboard; nada de mensagem técnica pro usuário.
export default function AdminError({
  ...
  const safeMessage = isSensitiveError(error?.message || '')
    ...
    : 'Não foi possível carregar esta página da Administração.'
