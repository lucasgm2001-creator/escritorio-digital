...
export function DayDetailModal({ dateStr, events, tasks, todayStr, onClose }: {
  ...
  return (
    ...
    <div onClick={onClose} className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center z-[300] p-0 sm:p-4">
      <div ref={ref} {...dialogProps} aria-labelledby="day-detail-title" onClick={e => e.stopPropagation()} className="bento-fx rounded-t-frame sm:rounded-frame shadow-card-hover w-full sm:max-w-md max-h-[85vh] overflow-y-auto animate-slide-up">
        <div className="flex items-center justify-between p-5 border-b border-bento-border">
          <h2 id="day-detail-title" className="font-display font-bold text-bento-text text-base capitalize">{titulo}</h2>
        ...
        <div className="p-5 space-y-4">
          {tasks.length > 0 && (
            <div className="space-y-1.5">
              <p className="font-tech text-[10px] uppercase tracking-[0.12em] text-bento-dim">Tarefas</p>
              {tasks.map(t => (
                <div key={t.id} className="flex items-center gap-2">
                  ...
                  {t.due_time && <span className="font-tech text-[11px] text-bento-dim flex-none">{t.due_time.slice(0, 5)}</span>}
                </div>
              ))}
            </div>
          )}
          {events.length > 0 && (
            <div className="space-y-1.5">
              <p className="font-tech text-[10px] uppercase tracking-[0.12em] text-bento-dim">Eventos</p>
              {events.map(e => (
                <div key={e.id} className="flex items-center gap-2">
                  ...
                  {e.start_time && <span className="font-tech text-[11px] text-bento-dim flex-none">{e.start_time.slice(0, 5)}</span>}
                </div>
              ))}
            </div>
          )}
