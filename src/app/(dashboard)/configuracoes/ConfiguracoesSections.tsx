...
// Seções de Configurações — extraídas VERBATIM do ConfiguracoesClient (UI-POLISH-GIANTS-001). Cada função é
// uma seção autossuficiente (estado/efeitos próprios); o ConfiguracoesClient continua sendo a casca de
// navegação (accordion/master-detail) que as renderiza por chave. Comportamento idêntico. Imports em superset
// (lint remove o que sobra). Componentes internos (ToggleRow/IntegStatus/cards/switches) ficam privados aqui.
import { useCallback, useEffect, useRef, useState } from 'react'
import { useRouter } from 'next/navigation'
import Image from 'next/image'
import {
  Sun, Moon, Monitor, Plug, Download, RefreshCw, ExternalLink, Workflow, ChevronRight, CalendarDays,
...
export function IntegracoesSection({ google }: { google: { connected: boolean; email: string | null } }) {
  const supabase = createClient()
...
export function AndarSection({ keyId, label, onOpenSub }: { keyId: string; label: string; onOpenSub: (sub: string) => void }) {
...
export function LogoUploadSection({ userId }: { userId: string }) {
...
export function HallSettingsSection({ userId }: { userId: string }) {
...
export function SegurancaSection({ role }: { role: 'owner' | 'admin' | 'member' }) {
...
export function TrafegoConfigSection() {
  return (
...
// (Props + a casca de navegação continuam no ConfiguracoesClient — este arquivo só implementa as seções.)
